if [ "$TARGETPLATFORM" = "linux/arm64" ]; then
    ...
    dotnet restore "./beholder-occipital-v1.csproj" --runtime linux-arm64
    ...
    dotnet restore "./beholder-occipital-v1.csproj" --runtime linux-arm
elif [ "$TARGETPLATFORM" = "linux/amd64" ]; then
    echo "Targeting linux/amd64";
    dotnet restore "./beholder-occipital-v1.csproj" --runtime linux-x64
else 
    echo "Targeting generic platform ($TARGETPLATFORM)";
    dotnet restore "./beholder-occipital-v1.csproj" -o /app/build
fi