namespace beholder_stalk
{
  ...
  /// </summary>
  public partial class Keyboard : HID
  {
    ...
    public Keyboard(IConfigurationRoot config)
        : base(config["hid:keyboard:devPath"])
    {
      int keyMin = 60;
      int keyMax = 170;

      if (int.TryParse(config["beholder_stalk_keymin"], out int configKeyMin))
      {
        keyMin = configKeyMin;
      }

      if (int.TryParse(config["beholder_stalk_keymax"], out int configKeyMax))
      {
        keyMax = configKeyMax;
      }

      AverageKeypressDuration = new Duration() { Min = keyMin, Max = keyMax };
      WatchNext();
    }

    public Duration AverageKeypressDuration
    {
      get;
      set;
    }
    ...
    private bool IsPressed(byte key, out int pressedKeyIndex)
    {
    ...
    {
      return SendKey(keypress.Key, keypress.Direction, keypress.Modifiers, keypress.Duration);
    }
    ...
    public async Task SendKeys(string keys)
    {
      ...
      foreach (Match m in KeysRegex.Matches(keys))
      {
        ...
        else if (m.Groups.ContainsKey("KeyName") && !string.IsNullOrWhiteSpace(m.Groups["KeyName"].Value))
        {
          ...
          {
            Key = keyName,
            Modifiers = GetModifiers(m, "KeyNameModifiers"),
            Duration = AverageKeypressDuration
          };
        }
        ...
        if (keypress != null)
        {
          if (m.Groups.ContainsKey("Direction"))
          {
            switch (m.Groups["Direction"].Value.ToLowerInvariant())
            {
              case "press":
              case "down":
                keypress.Direction = KeyDirection.Press;
                keypress.Duration = Duration.Infinite;
                break;
              case "release":
              case "up":
                keypress.Direction = KeyDirection.Release;
                keypress.Duration = Duration.Infinite;
                break;
              default:
                keypress.Direction = KeyDirection.PressAndRelease;
