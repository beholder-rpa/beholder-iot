if [ "$TARGETPLATFORM" = "linux/arm64" ]; then
    ...
    dotnet restore "./beholder-stalk-v2.csproj" --runtime linux-arm64
    ...
    dotnet restore "./beholder-stalk-v2.csproj" --runtime linux-arm
elif [ "$TARGETPLATFORM" = "linux/amd64" ]; then
    echo "Targeting linux/amd64";
    dotnet restore "./beholder-stalk-v2.csproj" --runtime linux-x64
else 
    echo "Targeting generic platform ($TARGETPLATFORM)";
    dotnet restore "./beholder-stalk-v2.csproj" -o /app/build
fi