namespace beholder_epidermis_v1
{
  ...
  public class PulseService : IHostedService, IDisposable
  {
    ...
    {
      _logger.LogInformation("Epidermis Pulse Service running.");
      ...
      _timer = new Timer(async state =>
      {
        try
        {
          ...
          _logger.LogInformation("Epidermis Pulsed");
        }
        catch (Exception ex)
        {
          _logger.LogError(ex, $"Error publishing epidermis pulse: {ex.Message}");
        }
    ...
    {
      _logger.LogInformation("Epidermis Pulse Service is stopping.");
